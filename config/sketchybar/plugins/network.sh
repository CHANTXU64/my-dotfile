...
DOWN_FORMAT=""
if [ "$DOWN" -gt "999" ]; then
  DOWN_FORMAT=$(echo $DOWN | awk '{ printf "%01.0f MBps", $1 / 1000 / 8}')
else
  DOWN_FORMAT=$(echo $DOWN | awk '{ printf "%01.0f kBps", $1 / 8}')
fi

UP_FORMAT=""
if [ "$UP" -gt "999" ]; then
  UP_FORMAT=$(echo $UP | awk '{ printf "%01.0f MBps", $1 / 1000 / 8}')
else
  UP_FORMAT=$(echo $UP | awk '{ printf "%01.0f kBps", $1 / 8}')
fi
